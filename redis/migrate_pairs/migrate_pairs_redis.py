# migrate_pairs_to_redis_full.py
...
for pair in pairs:
    ...
    pair_key = f"pair:{pair_doc['Номер_кабінету']}:{pair_doc['Дата']}:{pair_doc['Час_початку']}"

    # --- Зберігаємо у Redis як JSON рядок ---
    r.set(pair_key, json.dumps(pair_doc))

print(f"Вставка в Redis завершена! Загальна кількість пар: {len(pairs)}")
